    ...
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            reply_message = await message.reply_text(
                text=f"<b>नमस्कार {message.from_user.mention} 🙏🏻,\n\nतुम्हाला पाहिजे असलेला {search} हा चित्रपट मला सापडला आहे,\n\nतो चित्रपट मिळवण्यासाठी खालील button वर क्लिक करून आपल्या Auto Bot ग्रुपमध्ये सामील होऊन तो चित्रपट मिळवा ! 🙏🏻</b>",
...
# @Client.on_message((filters.group | filters.private) & filters.text & filters.incoming) # <<< Dont Uncomment >>>
# async def give_filter(client, message):
#     if message.chat.id != SUPPORT_CHAT_ID:
#         await message.react(emoji=random.choice(DS_REACT))
#         manual = await manual_filters(client, message)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/Marathi_Auto_Group")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
    ...
    # don't change anything without contacting me @Sanchit0102
        ...
        buttons = [[
            InlineKeyboardButton('𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐓𝐨 𝐌𝐚𝐤𝐞 𝐀𝐧𝐲 𝐁𝐨𝐭', url='https://t.me/THE_DS_OFFICIAL')
        ],[
            InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='start')
        ]]
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.DSBOT_TEXT.format(query.from_user.mention),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
